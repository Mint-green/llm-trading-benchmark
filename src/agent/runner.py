...
class AgentRunner(IAgentRunner):
    ...
    def __init__(
        ...
        custom_api_key = getattr(config, "custom_api_key", None)
        custom_api_base = getattr(config, "custom_api_base_url", None)
        custom_api_model = getattr(config, "custom_api_model", None)
        ...
        elif custom_api_key and custom_api_base:
            self._client = OpenAI(
                api_key=custom_api_key,
                base_url=custom_api_base,
                timeout=getattr(config, "custom_api_timeout", 180.0) or 180.0,
            )
            self._api_model = custom_api_model or model
            self._max_tokens = getattr(config, "custom_api_max_tokens", 4096) or 4096
        else:
            self._client = OpenAI(
                api_key=config.deepseek_api_key,
                base_url=config.deepseek_base_url,
