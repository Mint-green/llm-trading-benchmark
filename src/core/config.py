...
class RiskConfig:
    ...
    stop_loss_floor_pct: float = -0.05
    stop_loss_ceiling_pct: float = -0.10
    stop_loss_fixed_pct: float = -0.05
    daily_max_buys: int = 6
    circuit_breaker_consecutive_stops: int = 2


@dataclass(frozen=True)
class TieredSlippageConfig:
    """Tiered slippage surcharge for large trades."""
    enabled: bool = True
    small_max: float = 10_000
    small_surcharge: float = 0.0
    medium_max: float = 50_000
    medium_surcharge: float = 3.0
    large_max: float = 200_000
    large_surcharge: float = 8.0
    xlarge_surcharge: float = 15.0


@dataclass(frozen=True)
class LiquidityConfig:
    """Liquidity guard: limit trade size relative to market volume."""
    enabled: bool = True
...
class Config:
    ...
    # --- Data paths ---
    stock_data_dir: str = os.path.expanduser("~/Desktop/getStockData/data").replace("\\", "/")
    ...
    # --- LLM API (custom / any OpenAI-compatible provider) ---
    custom_api_key: str = ""
    custom_api_base_url: str = ""
    custom_api_model: str = ""
    custom_api_max_tokens: int = 4096
    custom_api_timeout: float = 180.0
    ...
    tiered_slippage: TieredSlippageConfig = field(default_factory=TieredSlippageConfig)
    ...
    def load_from_toml(cls, config_path: str = "config/config.toml") -> "Config":
        ...
        tiered_cfg = data.get("costs", {}).get("tiered_slippage", {})
        liquidity_cfg = data.get("portfolio", {}).get("liquidity", {})
        ...
        kwargs = {
            ...
            "risk": RiskConfig(
                stop_loss_mode=risk_cfg.get("stop_loss_mode", "fixed"),
                stop_loss_atr_multiple=risk_cfg.get("stop_loss_atr_multiple", 2.0),
                stop_loss_floor_pct=risk_cfg.get("stop_loss_floor_pct", -0.05),
                stop_loss_ceiling_pct=risk_cfg.get("stop_loss_ceiling_pct", -0.10),
                stop_loss_fixed_pct=risk_cfg.get("stop_loss_fixed_pct", -0.05),
                daily_max_buys=risk_cfg.get("daily_max_buys", 6),
                circuit_breaker_consecutive_stops=risk_cfg.get("circuit_breaker_consecutive_stops", 2),
            ),
            "tiered_slippage": TieredSlippageConfig(
                enabled=tiered_cfg.get("enabled", True),
                small_max=tiered_cfg.get("small_max", 10_000),
                small_surcharge=tiered_cfg.get("small_surcharge", 0.0),
                medium_max=tiered_cfg.get("medium_max", 50_000),
                medium_surcharge=tiered_cfg.get("medium_surcharge", 3.0),
                large_max=tiered_cfg.get("large_max", 200_000),
                large_surcharge=tiered_cfg.get("large_surcharge", 8.0),
                xlarge_surcharge=tiered_cfg.get("xlarge_surcharge", 15.0),
            ),
            "liquidity": LiquidityConfig(
                enabled=liquidity_cfg.get("enabled", True),
                max_pct_of_daily_volume=liquidity_cfg.get("max_pct_of_daily_volume", 5.0),
                lookback_days=liquidity_cfg.get("lookback_days", 1),
            ),
            "max_daily_trades": agent_cfg.get("max_daily_trades", 40),
        ...
        # Custom API provider
        custom_cfg = api_cfg.get("custom", {}) if isinstance(api_cfg, dict) else {}
        kwargs["custom_api_key"] = custom_cfg.get("api_key", "")
        kwargs["custom_api_base_url"] = custom_cfg.get("base_url", "")
        kwargs["custom_api_model"] = custom_cfg.get("model", "")
        kwargs["custom_api_max_tokens"] = custom_cfg.get("max_tokens", 4096)
        kwargs["custom_api_timeout"] = custom_cfg.get("timeout", 180.0)
